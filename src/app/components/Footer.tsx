...
const PLATFORM_LINKS = [
  { href: "/startups", label: "Barcha e'lonlar" },
...
export default function Footer() {
  ...
  return (
    <footer className="mt-auto border-t border-zinc-200 bg-zinc-50 dark:border-zinc-800/80 dark:bg-zinc-900/60">
      ...
      {/* ── Asosiy qism ── */}
      <div className="mx-auto max-w-7xl px-4 py-6 md:py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-4 md:gap-10 lg:gap-12">
          ...
          <div className="col-span-1 md:col-span-1">
            ...
            {/* Tavsif */}
            <p className="mb-5 max-w-xs text-xs md:text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
            ...
            <div className="flex items-center gap-2">
              {SOCIALS.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="flex h-8 w-8 md:h-9 md:w-9 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-500 transition-all duration-150 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:border-indigo-700 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-400"
                >
                  {s.icon}
                </a>
              ))}
            ...
            <ul className="space-y-1.5 md:space-y-2.5">
              {PLATFORM_LINKS.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="text-xs md:text-sm text-zinc-500 transition-colors hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"
                  >
                    {l.label}
                  </Link>
                </li>
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-3 md:py-5 sm:flex-row sm:px-6 lg:px-8">
