...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Fetch startups
        const startupsRes = await fetch(`${API_BASE_URL}/api/startups/`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (startupsRes.ok) {
  ...
  async function deleteStartup(id: number) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/startups/${id}/delete/`, {
        method: "DELETE",
        headers: {
  ...
  async function togglePremium(startupId: number) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/startups/${startupId}/toggle-premium/`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  async function toggleSoldStatus(startupId: number, currentStatus: boolean | undefined) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/startups/${startupId}/`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const SidebarLink = ({ item }: { item: NavItem }) => {
    ...
    const className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 ${
      activeTab === item.id
    ...
    if (item.href) {
      return (
        <Link href={item.href} className={className} onClick={() => setSidebarOpen(false)}>
          {content}
        </Link>
      );
    }
